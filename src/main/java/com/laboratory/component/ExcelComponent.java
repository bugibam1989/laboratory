...
public class ExcelComponent {
	...
	/**
	 * Merge cell A3 (colspan: 2, rowspan: 2) 타겟(시작셀)이름, COL,ROW
	 * @param st
	 * @return
	 */
	public String mergerInfo(Sheet st) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < st.getNumMergedRegions(); i++) {
			CellRangeAddress merge = st.getMergedRegion(i);
			CellRangeAddress mergedCell = new CellRangeAddress(merge.getFirstRow(), // first row (0-based)
	...
	public HashMap<String,Object> parse(MultipartFile file) throws Exception{
		...
		while(iter_sheet.hasNext()) {
			...
			String mergerInfo  = mergerInfo(sheet);
			rst.put("mergerInfo", mergerInfo);
		}// sheet Depth
